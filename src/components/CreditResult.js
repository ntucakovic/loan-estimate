...
class CreditResult extends Component {
  static getPropValue = propValue => {
    ...
    if (propValue) {
      const decimals =
        Math.round(propValue) === parseInt(propValue, 10) ? 0 : 2;
  ...
  static getPayoffDetails = (
    ...
    if (rate !== "-" && months !== "-") {
      return `${isUSD ? `${currencySymbol}` : ""}${rate}${
        !isUSD ? ` ${currencySymbol}` : ""
      } x ${months} ${localizationMonths}`;
    }
  ...
  render() {
    ...
    const showTotalAmount =
      (!calculation.totalAmountInput || calculation.totalAmountInput === 0) &&
    ...
    return (
      <div className="credit-result">
        <div className="form-field-group form-field-group--md-up">
          {showTotalAmount && (
            <label
              className={`credit-result__detail ${
                totalAmount ? "is-active" : ""
              } form-label form-label--with-prefix`}
              data-prefix={getSymbolFromCurrency(usedCurrency)}
            >
              {localization.totalAmount}
              <input
                className="credit-result__value form-input"
                type="text"
                disabled
                value={CreditResult.getPropValue(totalAmount)}
              />
            </label>
          )}
          ...
          >
            ...
            <input
              ...
              type="text"
              disabled
              value={CreditResult.getPropValue(loanTotal)}
            />
          </label>

          <label
            className={`credit-result__detail ${
              depositTotal ? "is-active" : ""
            } form-label form-label--with-prefix`}
            data-prefix={getSymbolFromCurrency(usedCurrency)}
          >
            ...
            <input
              ...
              type="text"
              disabled
              value={CreditResult.getPropValue(depositTotal)}
            />
          </label>
        </div>
        <label
          className={`credit-result__detail ${monthlyRate ? "is-active" : ""}`}
        >
          ...
          <input
            ...
            type="text"
            disabled
            value={CreditResult.getPayoffDetails(
              usedCurrency,
              monthlyRate,
              monthTotal,
              localization.months
